...
export default function DemoElderEmergency({ onNavigate }) {
    ...
    const triggerEmergency = () => {
        if (window.confirm("Calling Emergency Services... (Demo)")) {
            // Demo action
        }
        setProgress(0)
        setHolding(false)
        ...
        <div className="min-h-screen bg-cream-50 font-sans flex flex-col">
            ...
            <main className="flex-1 flex flex-col items-center justify-center px-6 pb-10">
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
                    ...
                    <p className="text-sage-600">Press and hold the button for 3 seconds to call for help.</p>
                </motion.div>

                {/* SOS Button */}
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.1 }} className="relative mb-6">
                    {/* Progress Ring */}
                    <svg className="absolute inset-0 w-full h-full -rotate-90" viewBox="0 0 200 200">
                        <circle cx="100" cy="100" r="90" fill="none" stroke="#FEE2E2" strokeWidth="8" />
                        <circle
                            cx="100"
                            cy="100"
                            r="90"
                    ...
                    >
                        ...
                        <span className="text-sm font-medium mt-1 uppercase tracking-wider">{holding ? 'Keep Holding...' : 'Hold to Call'}</span>
                    </motion.button>
                </motion.div>

                {/* Release Text */}
                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-rose-400 font-bold uppercase tracking-wider text-sm mb-10">
                    Release to Cancel
                </motion.p>

                {/* Emergency Contact Card */}
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="w-full max-w-sm bg-white rounded-2xl p-4 border border-sage-100 flex items-center gap-4">
